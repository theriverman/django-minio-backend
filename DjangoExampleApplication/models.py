...
from DjangoExampleApplication.storages import (get_public_storage, get_private_storage, get_filesystem_storage, get_image_storage)
...
class Image(models.Model):
    ...
    image = models.ImageField(upload_to=iso_date_prefix, storage=get_image_storage)

    @property
    def exists(self) -> bool:
        return self.image.storage.exists(self.file.name)
...
class PublicAttachment(models.Model):
    ...
    content_type: ContentType = models.ForeignKey(ContentType, null=True, blank=True, on_delete=models.CASCADE, verbose_name="Content Type")
    object_id = models.PositiveIntegerField(null=True, blank=True, verbose_name="Related Object's ID")
    content_object = GenericForeignKey("content_type", "object_id")

    file: FieldFile = models.FileField(verbose_name="Object Upload", storage=get_public_storage, upload_to=iso_date_prefix)
...
class FileSystemAttachment(models.Model):
    @property
    def file_name(self):
        return self.file.name.split("/")[-1]
    ...
    @property
    def file_size(self):
        return self.file.size if self.exists else 0

    def __str__(self):
        return str(self.file)

    id = models.AutoField(primary_key=True, verbose_name="File System Attachment ID")
    file: FieldFile = models.FileField(verbose_name="Object Upload", storage=get_filesystem_storage)
